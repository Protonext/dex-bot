 ...
 */
export class MarketMakerStrategy extends TradingStrategyBase {
    pairs = [];
    ...
    getBidAmountPerLevel(marketSymbol) {
    ...
    async prepareOrders(marketSymbol, marketDetails, openOrders) {
    ...
    createBuyOrder(marketSymbol, marketDetails, index) {
        const { market } = marketDetails;
        if (!market) {
            return undefined;
        }
    ...
    ;
    createSellOrder(marketSymbol, marketDetails, index) {
        const { market } = marketDetails;
        if (!market) {
            return undefined;
        }
