 ...
 */
export class GridBotStrategy extends TradingStrategyBase {
    oldOrders = [];
    pairs = [];
    ...
    async trade() {
        for (var i = 0; i < this.pairs.length; i++) {
            try {
                ...
                const upperLimit = new BN(this.pairs[i].upperLimit * 10 ** bidPrecision);
                const lowerLimit = new BN(this.pairs[i].lowerLimit * 10 ** bidPrecision);
                ...
                if (!this.oldOrders[i].length) {
                    ...
                    if (upperLimit.isGreaterThanOrEqualTo(priceTraded) && lowerLimit.isGreaterThanOrEqualTo(priceTraded))
                        maxGrids -= 1;
                    if (upperLimit.isLessThanOrEqualTo(priceTraded) && lowerLimit.isLessThanOrEqualTo(priceTraded))
                        index = 1;
                    var sellToken = 0;
                    var buyToken = 0;
                    for (; index <= maxGrids; index += 1) {
                        const price = upperLimit
                            ...
                            .dividedBy(10 ** bidPrecision)
                            .toFixed(askPrecision);
                        const { quantity, adjustedTotal } = this.getQuantityAndAdjustedTotal(price, bidAmountPerLevel, bidPrecision, askPrecision);
                        const validOrder = new BN(Math.abs(parseFloat(price) - parseFloat(lastSalePrice))).isGreaterThanOrEqualTo(+gridPrice / 2);
                        const validBuyOrder = new BN((parseFloat(lastSalePrice) - parseFloat(price))).isGreaterThan(0);
                        const validSellOrder = new BN((parseFloat(price) - parseFloat(lastSalePrice))).isGreaterThan(0);
                    ...
                    let currentOrders = openOrders.map((order) => ({
                    ...
                    for (var j = 0; j < this.oldOrders[i].length; j++) {
                        const newOrder = openOrders.find((openOrders) => openOrders.price === this.oldOrders[i][j].price);
                        if (!newOrder) {
                            ...
                            if (this.oldOrders[i][j].orderSide === ORDERSIDES.BUY) {
                                ...
                                if (!lowestAsk || config.gridPlacement)
                                    ...
                                    sellPrice = new BN(lowestAsk)
                                        .minus(gridPrice)
                                        .toFixed(askPrecision);
                                const { quantity } = this.getQuantityAndAdjustedTotal(sellPrice, bidAmountPerLevel, bidPrecision, askPrecision);
                                const order = {
                                    orderSide: ORDERSIDES.SELL,
                                    price: +sellPrice,
                                    quantity: quantity,
                                ...
                                const { adjustedTotal } = this.getQuantityAndAdjustedTotal(buyPrice, adjustedBidAmountPerLevel, bidPrecision, askPrecision);
                                const order = {
                                    orderSide: ORDERSIDES.BUY,
                                    price: +buyPrice,
                                    quantity: adjustedTotal,
...
//# sourceMappingURL=gridbot-split-return-preference.js.map